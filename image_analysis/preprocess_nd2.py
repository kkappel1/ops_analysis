...
def GFP_nd2_to_tif_rotate_180_and_align_resize(input_filename, output_dir, ch_dapi, ch_gfp, base_align_c1_dir ):
    with ND2_Reader(input_filename) as images:
        images.iter_axes = 'm'
        images.bundle_axes = 'cyx'
        ...
        for site, image in enumerate(images):
            ...
            image = image.astype('uint16')
            image = image[[ch_dapi, ch_gfp],:]
            image = skimage.measure.block_reduce( image, block_size=(1,2,2), func=np.mean)
